...
describe("diagnostics-otel service", () => {
  ...
  test("redacts sensitive data from log messages before export", async () => {
    const emitCall = await emitAndCaptureLog({
      level: "INFO",
      message: "Using API key REDACTED_HISTORICAL_SECRET1234567890abcdef",
    });

    expect(emitCall?.body).not.toContain("REDACTED_HISTORICAL_SECRET1234567890abcdef");
  ...
  test("bounds plugin-emitted log attributes and omits source paths", async () => {
    ...
    attributes["REDACTED_HISTORICAL_SECRET1234567890abcdef"] = "secret-key"; // pragma: allowlist secret
    ...
    expect(
      Object.hasOwn(
        emitCall?.attributes ?? {},
        "openclaw.REDACTED_HISTORICAL_SECRET1234567890abcdef", // pragma: allowlist secret
      ),
    ).toBe(false);
    expect(emitCall?.attributes).toEqual(
      expect.not.objectContaining({
  ...
  test("exports bounded redacted content when capture fields are opted in", async () => {
    ...
    emitDiagnosticEvent({
      ...
      model: "gpt-5.4",
      durationMs: 80,
      inputMessages: ["use key REDACTED_HISTORICAL_SECRET1234567890abcdef"], // pragma: allowlist secret
    ...
    expect(String(modelAttrs?.["openclaw.content.input_messages"])).not.toContain(
      "REDACTED_HISTORICAL_SECRET1234567890abcdef", // pragma: allowlist secret
    );
