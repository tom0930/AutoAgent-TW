...
describe("redactSensitiveText", () => {
  ...
  it("redacts large payloads with bounded regex passes", () => {
    const input = `${"x".repeat(40_000)} OPENAI_API_KEY=REDACTED_HISTORICAL_SECRET ${"y".repeat(40_000)}`;
    const output = redactSensitiveText(input, {
      mode: "tools",
      patterns: defaults,
    });
  ...
  it("honors logging redaction settings from the active config path", () => {
    ...
    expect(redactSensitiveText("OPENAI_API_KEY=REDACTED_HISTORICAL_SECRET")).toBe(
      "OPENAI_API_KEY=REDACTED_HISTORICAL_SECRET",
    );
  });
  ...
  it("does not resolve patterns when mode is off", () => {
    ...
    expect(redactSensitiveText("OPENAI_API_KEY=REDACTED_HISTORICAL_SECRET", options)).toBe(
      "OPENAI_API_KEY=REDACTED_HISTORICAL_SECRET",
    );
  });
