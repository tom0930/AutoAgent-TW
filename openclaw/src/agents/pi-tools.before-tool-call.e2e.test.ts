...
describe("before_tool_call loop detection behavior", () => {
  ...
  it("emits diagnostic tool execution events without parameter values", async () => {
    ...
    await withToolExecutionEvents(async (emitted, flush) => {
      await tool.execute(
        "tool-call-1",
        { command: "pwd", token: "REDACTED_HISTORICAL_SECRET1234567890abcdef" },
        undefined,
        undefined,
      );
      await flush();
      ...
      expect(JSON.stringify(emitted)).not.toContain("REDACTED_HISTORICAL_SECRET1234567890abcdef");
  ...
  it("emits diagnostic tool execution error events with redacted errors", async () => {
    const execute = vi
      .fn()
      .mockRejectedValue(new Error("failed with key REDACTED_HISTORICAL_SECRET1234567890abcdef"));
    ...
    await withToolExecutionEvents(async (emitted, flush) => {
      ...
      expect(JSON.stringify(emitted[1])).not.toContain("REDACTED_HISTORICAL_SECRET1234567890abcdef");
    });
  });
