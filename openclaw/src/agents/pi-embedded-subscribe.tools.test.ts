...
describe("sanitizeToolResult", () => {
  it("redacts JSON-style apiKey fields in text content blocks", () => {
    const result = {
      content: [
        {
          type: "text",
          text: '{"apiKey":"REDACTED_HISTORICAL_SECRET","model":"gpt-4"}',
        },
      ],
    };
    const text = getTextContent(sanitizeToolResult(result));
    expect(text).not.toContain("REDACTED_HISTORICAL_SECRET");
  ...
  it("redacts a details-only result with no content array", () => {
    const result = {
      details: {
        config: { apiKey: "REDACTED_HISTORICAL_SECRETXYZ", model: "gpt-4" },
      },
    };
    const sanitized = sanitizeToolResult(result) as {
      details: { config: { apiKey: string; model: string } };
    };
    expect(sanitized.details.config.apiKey).not.toContain("REDACTED_HISTORICAL_SECRETXYZ");
  ...
  it("preserves top-level arrays while redacting nested strings", () => {
    const sanitized = sanitizeToolResult([
      ...
      "apiKey=REDACTED_HISTORICAL_SECRETXYZ",
    ]) as Array<{ output: string } | string>;

    expect(Array.isArray(sanitized)).toBe(true);
    expect(JSON.stringify(sanitized)).not.toContain("abcdef0123456789QWERTY=");
    expect(JSON.stringify(sanitized)).not.toContain("REDACTED_HISTORICAL_SECRETXYZ");
...
describe("sanitizeToolArgs", () => {
  it("redacts string-valued credentials nested anywhere in args", () => {
    const args = {
      ...
      flags: ["--api-key", "REDACTED_HISTORICAL_SECRETXYZ"],
    };
    ...
    expect(sanitized.apiKey).not.toContain("REDACTED_HISTORICAL_SECRETXYZ");
    ...
    expect(sanitized.flags.join(" ")).not.toContain("REDACTED_HISTORICAL_SECRETXYZ");
