...
export async function ensureConfigReady(params: {
  ...
  if (process.env.OPENCLAW_SKIP_CONFIG_GUARD === "1") {
    const [{ colorize, isRich, theme }] = await Promise.all([
      import("../../terminal/theme.js"),
    ]);
    if (isRich() && !params.suppressDoctorStdout) {
      params.runtime.log(colorize(true, theme.warning, " ✦ Antigravity Standalone Mode Active (Offline) ✦ "));
    }
    return;
  }
